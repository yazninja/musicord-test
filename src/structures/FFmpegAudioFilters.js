...
exports.FFmpegAudioFilters = void 0;
class FFmpegAudioFilters {
    ...
    pingPongDelay(value) {
        if (!value || typeof value !== 'number')
            throw new TypeError('The time limit as a number is required');
    ...
    volume(value) {
        if (!value || typeof value !== 'number')
            throw new TypeError('Volume as a number is required');
    ...
    customEqualizer(options) {
        ...
        for (const [band, gain] of Object.entries(Object(options)))
            toReturn += `${Number(String(band).replace('band', '')) == 1
                ? '1b=' + Number(gain) / 100 + ':'
    ...
    customFilter(filter) {
        if (!filter || typeof filter !== 'string')
            throw new TypeError('A textual value is required');
        return filter;
    }
    /**
...
exports.FFmpegAudioFilters = FFmpegAudioFilters;
//# sourceMappingURL=FFmpegAudioFilters.js.map